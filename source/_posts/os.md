...
-----------------
进程内的线程切换 -> 普通线程切换  保存线程现场状态（寄存器、栈指针）

线程切换是操作系统中的一种核心机制，用于在多线程环境下实现并发执行。下面是典型的线程切换流程：

当前线程执行到一个可以切换出去的状态时，例如等待 I/O、睡眠、自愿让出 CPU 等。

操作系统内核保存当前线程的上下文信息，包括 CPU 寄存器的值、栈指针、程序计数器等。

操作系统从线程队列中选择下一个要执行的线程。

操作系统恢复下一个线程的上下文信息，并将控制权转移到该线程。

新的线程开始执行，直到它再次被切换出去。

如果之前的线程仍然在队列中，它将等待再次被选中执行，并从之前被保存的上下文信息处继续执行。

以上流程可以循环执行，直到所有的线程都执行完毕或者操作系统关闭。在实现线程切换的过程中，关键在于操作系统如何有效地保存和恢复线程上下文信息，以及如何选择下一个要执行的线程。此外，线程切换也会带来一定的开销，因为需要保存和恢复上下文信息，所以需要权衡线程切换的频率和性能开销。
